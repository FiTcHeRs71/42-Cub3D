...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/18 10:50:36 by fducrot           #+#    #+#             */
/*   Updated: 2026/05/18 10:50:36 by fducrot          ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_bonus_textures(t_data *data, t_texture *texture, t_mlx *mlx)
{
	...
	texture->ground.addr = mlx_get_data_addr(texture->ground.img,
			&texture->ground.bpp, &texture->ground.size_line,
			&texture->ground.endian);
	texture->ceiling.img = mlx_xpm_file_to_image(mlx->mlx_connect,
			"./images/ceiling_floor/ceiling.xpm", &texture->ceiling.width,
			&texture->ceiling.height);
	...
	texture->ceiling.addr = mlx_get_data_addr(texture->ceiling.img,
			&texture->ceiling.bpp, &texture->ceiling.size_line,
			&texture->ceiling.endian);
}
