...
void	load_bonus_textures(t_data *data, t_texture *texture, t_mlx *mlx)
{
	...
	texture->floor.img = mlx_xpm_file_to_image(mlx->mlx_connect,
			"./images/ceiling_floor/floor.xpm",
			&texture->floor.width, &texture->floor.height);
	if (!texture->floor.img)
		ft_error_ctx("Texture", "./images/ceiling_floor/floor.xpm", data);
	texture->floor.addr = mlx_get_data_addr(texture->floor.img,
			&texture->floor.bpp, &texture->floor.size_line, &texture->floor.endian);
